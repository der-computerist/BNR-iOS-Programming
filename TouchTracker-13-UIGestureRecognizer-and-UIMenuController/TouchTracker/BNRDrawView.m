...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    for (UITouch *t in touches) {
        ...
        line.width = [self getLineWidthForCurrentVelocity];
...
- (void)longPress:(UIGestureRecognizer *)gr
{
    ...
    NSLog(@"Long Press triggered, its current state is: %ld", (long)gr.state);
...
- (void)moveLine:(UIPanGestureRecognizer *)gr
{
    ...
    // Record the current pan's velocity for the purpose of line drawing
    self.currentVelocity = [gr velocityInView:self];
    ...
    // If there is not an ongoing long press, we do not anything either
    if (!(self.pressRecognizer.state == UIGestureRecognizerStateBegan
        || self.pressRecognizer.state == UIGestureRecognizerStateChanged)) {
        return;
    }
...
- (CGFloat)getLineWidthForCurrentVelocity
{
    CGFloat speed = hypotf(self.currentVelocity.x, self.currentVelocity.y);
    CGFloat lineWidth = speed / 100.0;
    return lineWidth;
}
